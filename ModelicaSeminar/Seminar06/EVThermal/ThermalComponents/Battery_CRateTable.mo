...
model Battery_CRateTable "simple battery from file"
  ...
  .Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow
    ...
    .LiquidCooling.FlowResistances.Generic.LiquidResistance flowResistance(redeclare
      replaceable package                                                                                Medium = Medium) annotation(Placement(transformation(extent = {{-65.96149934356244,-9.853800157017867},{-45.96149934356242,10.146199842982133}},origin = {0.0,0.0},rotation = 0.0)));
  ...
  .Modelica.Blocks.Sources.RealExpression batteryTemp_degC(y=.Modelica.Units.Conversions.to_degC(
    ...
    replaceable .Electrification.Batteries.Examples.Lumped battery(accumulated_outputs = true,redeclare replaceable .Electrification.Batteries.Core.Examples.Nominal core(specify_losses = Electrification.Batteries.Utilities.Types.SpecLosses.Heat),redeclare replaceable .Electrification.Batteries.Thermal.Examples.Lumped thermal,redeclare replaceable .Electrification.Batteries.Control.LimitsAndContactors controller) annotation(Placement(transformation(extent = {{-60.0,38.0},{-40.0,58.0}},origin = {0.0,0.0},rotation = 0.0)));
equation
  ...
  connect(batteryCoolant_outT_degC.y, controlBus.batteryCoolantOutT_degC)
    annotation (Line(points={{38.3,80},{0,80},{0,98}}, color={0,0,127}), Text(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
    connect(prescribedHeatFlow.Q_flow,controlBus.Q_battery) annotation(Line(points = {{2.220446049250313e-15,40},{0,98}},color = {0,0,127}));
end Battery_CRateTable;
