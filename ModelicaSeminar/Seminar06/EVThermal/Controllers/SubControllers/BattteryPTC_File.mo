...
model BattteryPTC_File "battery PTC controller, file based"
  ...
  .Modelica.Blocks.Sources.CombiTimeTable T_PTC_target_degC(
    tableOnFile=true,
    tableName="T_PTC_battery_target_degC",
...
equation
  connect(T_PTC_target_degC.y[1], controlBus.T_PTC_target_battery) annotation (
  ...
  connect(greaterThreshold.y, controlBus.PTC_enable_battery) annotation (Line(
        points={{1,-70},{80,-70},{80,0},{100,0}}, color={255,0,255}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(realPassThrough.u, controlBus.T_PTC_coolant_battery_degC) annotation (
     Line(points={{62,50},{80,50},{80,0},{100,0}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(realPassThrough.y, controlBus.T_PTC_sensed_battery) annotation (Line(
        points={{39,50},{20,50},{20,0},{100,0}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
end BattteryPTC_File;
