...
model HXLLumpedToACL "Adapter for HXL lumped air port to ACL air port"
...
<hr><p><font color=\"#E72614\"><b>Copyright &copy; 2004-2018, MODELON AB</b></font> <font color=\"#AFAFAF\">The use of this software component is regulated by the licensing conditions for the ThermoFluidPro Library. This copyright notice must, unaltered, accompany all components that are derived from, copied from, or by other means have their origin from the ThermoFluidPro Library. </font>
  ...
  .Modelon.ThermoFluid.Interfaces.ApplicationSpecific.AirFlowPort portA(redeclare
      package Medium = MediumHXL)
    "Upstream port"
...
equation
  ...
  inStream(portA.h_outflow) = MediumHXL.specificEnthalpy(MediumHXL.setState_pTX(
    p=portA.p,
    T=Tair,
  ...
  portB.h_outflow = MediumACL.specificEnthalpy(MediumACL.setState_pTX(
    p=portB.p,
    T=Tair,
    X=portB.X_outflow));

  // Properties for reverse flow (not allowed)
  portA.h_outflow = MediumHXL.h_default;
  portA.X_outflow = MediumHXL.reference_X;
